...
function Player() {
  ...
  useEffect(() => {
    ...
    if (loadingState.isLoaded & isPlaying) {
      ...
      let animatingContent = (fbc_array) => {
        ...
        gsap.to(light, 0.05, {
          scaleX: 0.5 + fbc_array[1] / 4 / 100,
          scaleY: 0.5 + fbc_array[99] / 4 / 100,
        })
    ...
    return () => {
      gsap.to(".player__line", 0.5, { width: 0 })
      if (!isMobile()) {
        gsap.to(".light", 0.5, { scaleX: 0.5, scaleY: 0.5 })
        gsap.to(".home__welcome--shadow", 0.5, { scale: 1.05 })
      }
  ...
  if (isPlaying) {
    content = (
      <i
        onMouseOver={cursorExpand}
        onMouseOut={cursorBackToNormal}
        onClick={playHandle}
  ...
  return (
    <div className="player">
      <div className="player__content">{content}</div>
      <a
        draggable={false}
        href="https://soundcloud.com/halfaliveco/still-feel"
        target="_blank"
        rel="noopener noreferrer"
      >
        <span className="player__song">
          <span>still feel - half•alive</span>
        </span>
      </a>
    </div>
  )
